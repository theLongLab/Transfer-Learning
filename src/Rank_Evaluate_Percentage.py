...
def process(dis, batch, SnpNumsString , model, StatesIndex, log):
    ...
    for chr_ in range(1,23):
        perchr=pd.read_parquet("/work/long_lab/qli/Enformer_DTL/HG19_UKB_DTL_Mto1_ENF_Mto1_ABS_scores_10M/"+dis+"/"+batch+"/"+str(chr_)+"."+model+".NumberTracks.parquet")
    ...
    for SnpNum in SnpNumsList:
        ...
        for i in StatesIndexList:
            ...
            for CellLine in cell_lines:           
                SNP_in_func_regions,SNP_outof_func_regions,records_df_perCellLine = number_SNPs_in_out_of_func_regions(model_output_df_mean_sorted_topSnpNum_snps_ID,CellLine,ChromState)
...
if __name__ == "__main__":
    ...
    dis=sys.argv[3]
    batch=sys.argv[4]
    StatesIndex="6_7_11_12" #0 for 15 states. Otherwise, join by _, e.g. 6_7
